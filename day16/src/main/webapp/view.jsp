<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<form method="POST" action="reply-write.jsp">
		...
		<c:if test="${not empty login }">
			<c:set var="replyComment">불편해? 불편하면 자세를 고쳐 앉아</c:set>
		</c:if>
		...
		<c:forEach var="reply" items="${replyDAO.selectList(param.idx) }">
			<div class="reply">
				...
				<c:if test="${login.userid == reply.writer }">
					<div><a class="deleteLink" href="${cpath }/delete.jsp?idx=${reply.idx }&board_idx=${dto.idx}"><button>댓글 삭제</button></a></div>			
				</c:if>
			</div>
	...
	<script type="text/javascript">
					function deleteHandler(event) {
						...
						const flag = confirm('정말 댓글을 삭제하시겠습니까?')
						if(flag){
